...
if (isset($_GET['id'])) {
  ...
  // Update the employee status to 'Archive'
  $stmt = $conn->prepare("UPDATE empreco SET status = 'Archive' WHERE Employee_ID = ?");
...
$archived_sql = "SELECT * FROM empreco WHERE status = 'Archive'";
$archived_result = $conn->query($archived_sql);

// Restore employee functionality
if (isset($_GET['restore_id']) && isset($_GET['target_status'])) {
  $restore_id = intval($_GET['restore_id']);
  $target_status = $_GET['target_status']; // 'active' o 'resigned'

  // Validate the target_status
  switch ($target_status) {
      case 'active':
          $new_status = 'Not Resigned';  // Change to 'Active' if needed
          break;
      case 'resigned':
          $new_status = 'Resigned';
          break;
      default:
          ...
          $_SESSION['restore_message'] = 'Invalid target status.';
  ...
  if ($stmt->execute()) {
      $_SESSION['restore_message'] = 'Employee restored successfully!';
  } else {
      $_SESSION['restore_message'] = 'Error restoring employee: ' . $conn->error;
  }
...
<div class="dropdown">
  ...
  <div class="dropdown-content">
    ...
    <a href="archive_two.php?status=not_resigned">Active</a>
  </div>
</div>
 ...
 <section class="main">
    <div class="main--content">
        <div class="recent--patients">
            <h2 class="section--title">Archived Active Employees</h2>
  ...
  if ($archived_result->num_rows > 0) {
      while ($row = $archived_result->fetch_assoc()) {
          ...
          echo "<td><a href='archive_two.php?restore_id=" . $row['Employee_ID'] . "&target_status=active' class='btn btn-primary'>Restore</a></td>";
          echo "</tr>";
      }
  } else {
